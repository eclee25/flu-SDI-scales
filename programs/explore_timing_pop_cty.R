...
for(s in seasons){
    ...
    onsetScatter3 <- ggplot(dummyOnset, aes(x = onsetwk_cty, y = pop, group = season)) +
        geom_point(aes(colour = relative_to_st), alpha = 0.7) +
        scale_y_log10("County Population") +
        scale_x_date("County Onset Week", date_breaks = "1 month", date_labels = "%b") +
        scale_colour_manual("Relative to state onset week", values = pal) +
        ...
        theme(legend.position = "bottom", axis.text.x = element_text(angle = 45, hjust = 1)) +
        facet_wrap(~state)
    ggsave(glue::glue("../graph_outputs/explore_timing_pop_cty/onset_scatter_st_S{s}.png"), onsetScatter3, width = 7, height = 8)

    dummyPeak <- dplyr::filter(peakDat, season == s)
    peakScatter3 <- ggplot(dummyPeak, aes(x = peakwk_cty, y = pop, group = season)) +
        geom_point(aes(colour = relative_to_st), alpha = 0.7) +
        scale_y_log10("County Population") +
        scale_x_date("County Peak Week", date_breaks = "1 month", date_labels = "%b") +
        scale_colour_manual("Relative to state peak week", values = pal) +
        ...
        theme(legend.position = "bottom", axis.text.x = element_text(angle = 45, hjust = 1)) +
        facet_wrap(~state)
    ggsave(glue::glue("../graph_outputs/explore_timing_pop_cty/peak_scatter_st_S{s}.png"), peakScatter3, width = 7, height = 8)
}