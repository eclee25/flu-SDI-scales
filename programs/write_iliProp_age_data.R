...
agegroup <- "adult" # don't toggle

#### import data ################################
setwd('../Py_export')
ili_df <- read.csv(sprintf('iliByallZip_allWeekly_totServ_%s.csv', agegroup), header=T, colClasses=c("week"="Date"))
viz_df <- read.csv(sprintf('vizByallZip_allWeekly_totServ_%s.csv', agegroup), header=T, colClasses=c("week"="Date"))
...
ili_gather_df <- gather(ili_df2, zip3, ili, X2:X999, convert=FALSE) %>% 
  ...
  mutate(ili = ifelse(is.na(ili), 0, ili)) # 7/15/16: replace NAs with 
...
# merge ili & pop data
fullDat <- left_join(merge_gather_df, pop_data, by = c('zip3', 'year'))
