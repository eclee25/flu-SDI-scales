...
## Filenames: periodicReg_%sallSCALEMods_irDt_Octfit_span#_degree#.csv
...
write_relativeDiseaseBurden_irDt <- function(span.var, degree.var, spatial){
  ...
  if (spatial$scale == 'state'){
    ...
    setwd('../R_export')
    importData <- read_csv(sprintf('fullIndicAll_periodicReg_%sirDt%s%s%s%s_analyzeDB%s.csv', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr), col_names = T, col_types = list(fips = col_character(), pop = col_integer(), .fitted = col_double(), .se.fit = col_double(), .fittedLoess = col_double(), .se.fitLoess = col_double(), ir.dt = col_double(), iliProp = col_double(), in.season = col_logical(), has.epi = col_logical(), incl.analysis = col_logical())) %>%
      rename(scale = fips)
    ...
    setwd('../R_export')
    importData <- read_csv(sprintf('fullIndicAll_periodicReg_%sirDt%s%s%s%s_analyzeDB%s.csv', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr), col_names = T, col_types = list(region = col_character(), ili = col_double(), viz = col_double(), pop = col_integer(), .fitted = col_double(), .se.fit = col_double(), .fittedLoess = col_double(), .se.fitLoess = col_double(), ir.dt = col_double(), iliProp = col_double(), in.season = col_logical(), has.epi = col_logical(), incl.analysis = col_logical())) %>%
      rename(scale = region)
  ...
  setwd('../reference_data')
  earlySeasonData <- read_csv("earlySeasonDefinition_irDt_expRate.csv", col_types = "_Dl")
  ...
  dbMetrics.epi <- data5 %>% 
    ...
    summarize(has.epi = first(has.epi), incl.analysis = first(incl.analysis), 
      ...
      irDt.early = sum(ifelse(in.earlyseason, ir.dt, 0), na.rm=T), 
  ...
  dbMetrics.noepi <- data5 %>% 
    ...
    summarize(has.epi = first(has.epi), incl.analysis = first(incl.analysis), 
      ...
      irDt.early = sum(irDt.modified, na.rm = T),
  ...
  dbMetrics.noisy <- data5 %>% 
    ...
    summarize(has.epi = first(has.epi), incl.analysis = first(incl.analysis), 
      ...
      irDt.early = first(irDt.modified), 
  ...
  data7 <- data5 %>% select(Thu.week, t, ir.dt, season, scale, flu.week, in.season)
  createTiming1 <- data7 %>% 
    group_by(season, scale) %>% 
  ...
  write.csv(dbMetrics.g2, file = sprintf('dbMetrics_periodicReg_%sirDt%s%s%s%s_analyzeDB%s.csv', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr), row.names=FALSE)
  
  # save summary data to file with coords 
  # these data are used in "analyze_dbMetricsDistance_IR.R" for matrix correlations
  setwd('../R_export')
  write.csv(dbMetrics.coords2, file = sprintf('dbMetrics_periodicReg_%sirDt%s%s%s%s_analyzeDBdist%s.csv', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr), row.names=FALSE)
