...
model8h_iliEarly_v7 <- function(filepathList){
  # 9/15/17 model 8h_v7 -- total population
  # multi-season model, shifted1 iliEarly response, all sampling effort, and driver variables
...
model8b_iliPeak_v7 <- function(filepathList){
  # 9/15/17 model 8b_v7 -- total population
  # multi-season model, shifted1 iliPeak response, all sampling effort, and driver variables
  # with 2 spatial terms - state flight passenger, county neighbor
  # y1 = non-zero response, E = expected response
  print(match.call())
  print(filepathList)
...
model8i_wksToPeak_v7 <- function(filepathList){
  # 3/31/17 model 8i_v7 -- total population
  ...
  #### import data ####
  # IMS Health based tables
  mod_cty_df <- cleanR_wksToPeak_cty(filepathList)
  imsCov_cty_df <- cleanO_imsCoverage_cty()
  imsCareseek_cty_df <- cleanO_imsCareseekTot_cty() # 1/5/17 visitsPerPop from sdi flu data
  # all county tables
  sahieIns_cty_df <- cleanO_sahieInsured_cty()
