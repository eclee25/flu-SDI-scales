...
explore_periodicReg_inSeasonFits_irDt <- function(span.var, degree.var, spatial){
  ...
  if (spatial$scale == 'state'){
    ...
    data5 <- read_csv(sprintf('fullIndicAll_periodicReg_%sirDt%s%s%s%s_analyzeDB%s.csv', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr), col_names = T, col_types = list(national = col_character(), ili = col_double(), viz = col_double(), pop = col_integer(), .fitted = col_double(), .se.fit = col_double(), .fittedLoess = col_double(), .se.fitLoess = col_double(), ir.dt = col_double(), IR = col_double())) %>%
      rename(scale = national)
  }
  
  #### 9/15/15 in.season fits ################################
  print(sprintf('plotting in season time series %s', code.str))
  dir.create(sprintf('../graph_outputs/explore_periodicReg_%sfits_irDt%s%s%s%s%s/inSeason', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr), showWarnings = FALSE)
  setwd(sprintf('../graph_outputs/explore_periodicReg_%sfits_irDt%s%s%s%s%s/inSeason', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr))
  ...
  for(i in indexes2){
    ...
    ggsave(sprintf("periodicReg_inSeas_%sfits_irDt%s%s_%s-%s.png", code, code2, code.str, ziplabels[1,], ziplabels[2,]), dummyplots, width=w, height=h)
  } 
