...
# 
write_fullIndic_periodicReg_irDt <- function(span.var, degree.var, spatial){
...
#   degree.var <- 2
  ...
  if (spatial$scale == 'zip3'){
    ...
    data <- read_csv(file=sprintf('periodicReg_%sall%sMods_irDt%s%s.csv', code, spatial$stringcode, code2, code.str), col_types=list(region = col_character(), ili = col_double(), viz = col_double(), pop = col_double(), .fitted = col_double(), .se.fit = col_double(), .fittedLoess = col_double(), .se.fitLoess = col_double(), ir.dt = col_double(), iliProp = col_double())) %>%
      rename(scale = region)
    num.weeks <- 2 
  } else if (spatial$scale == 'national'){
    data <- read_csv(file=sprintf('periodicReg_%sall%sMods_irDt%s%s.csv', code, spatial$stringcode, code2, code.str), col_types=list(national = col_character(), ili = col_double(), viz = col_double(), pop = col_double(), .fitted = col_double(), .se.fit = col_double(), .fittedLoess = col_double(), .se.fitLoess = col_double(), ir.dt = col_double(), iliProp = col_double())) %>%
  ...
  data3 <- data2 %>% filter(incl.lm) %>% mutate(epi.thresh = ifelse(incl.lm2, .fitted+(1.96*.se.fit), 0)) %>% mutate(epi.week = ir.dt>epi.thresh)
  ...
  data5 <- data4 %>% filter(flu.week & has.epi & incl.analysis) %>% group_by(season, scale) %>% mutate(in.season = consider.flu.season_maxconsec(epi.week))
  View(data5)
  ...
  write.csv(data5_write, file = sprintf('fullIndicFlu_periodicReg_%sirDt%s%s%s%s_analyzeDB%s_dzBurdenMS.csv', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr), row.names=FALSE)
  write.csv(data6_write, file = sprintf('fullIndicAll_periodicReg_%sirDt%s%s%s%s_analyzeDB%s_dzBurdenMS.csv', code, code2, spatial$servToggle, spatial$ageToggle, code.str, spatial$stringabbr), row.names=FALSE)
